...
int
argstructptr(int n, struct iostats **pp, int size)
{
  int i;
...
sys_read(void)
{
  ...
  curproc->ios[fd]->read_bytes += read_bytes; 
  return read_bytes;
}
...
sys_write(void)
{
  ...
  curproc->ios[fd]->write_bytes += write_bytes; 
  return write_bytes;
}
...
sys_getiostats(void)
{
 ...
 argfd(0, &fd, &f); // get the fd and file
